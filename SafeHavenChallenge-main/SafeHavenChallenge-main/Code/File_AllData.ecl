...
EXPORT CampRec :=  RECORD
    ...
    STRING amenities;	
    STRING state;	
    UNSIGNED nearest_town_distance;	
    STRING nearest_town_bearing;	
    STRING city;
END;
EXPORT CampDS := DATASET('~anonymous_solvers::safe_camp::us_campsites.csv',CampRec,CSV(HEADING(1)));

EXPORT UniversityRec:=RECORD
    REAL8 X	;
    REAL8 Y	;
    INTEGER FID	;
    INTEGER IPEDSID;
    ...
    STRING SHELTER_ID;
  END;
EXPORT UniversityDS:=DATASET('~anonymous_solvers::safeuni::unidata_unnathics.csv',UniversityRec,CSV(HEADING(1)));
